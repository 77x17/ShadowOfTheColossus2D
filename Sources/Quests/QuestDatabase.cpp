...
std::vector<Quest> QuestDatabase::quests;

void QuestDatabase::loadQuests() {
    quests.clear();
...
const Quest& QuestDatabase::getQuestByID(const int& questID) {
    if (questID >= static_cast<int>(quests.size())) {
        std::cerr << "[Bug] - QuestDatabase.cpp - getQuestByID()\n";
    }

    return quests[questID];
}

const std::vector<Quest>& QuestDatabase::getAllQuests() {
    return quests;
}

const int QuestDatabase::getQuestsSize() {
    return quests.size();
}